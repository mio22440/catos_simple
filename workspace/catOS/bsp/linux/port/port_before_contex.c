...
static void _cat_linux_tick_signal_handler(int sig)
{
    ...
    )
    {
        if (0 == pthread_mutex_trylock(&cat_linux_single_thread_mutex))
        {
            ...
            result = _cat_linux_lookup_thread(cat_sp_cur_task, &thread_to_suspend);
            CAT_ASSERT(0 == result);
            ...
            result = _cat_linux_lookup_thread(cat_sp_cur_task, &thread_to_resume);
            CAT_ASSERT(0 == result);
...
static void _cat_linux_redeal_missed_tick(void)
{
    ...
    result = LookupThread(cat_sp_cur_task, &thread_to_resume);
    CAT_ASSERT(0 == result);
...
void catos_start_sched(void)
{
    ...
    while(1 != cat_linux_is_scheduler_end)
    {
        ...
        {
            printf("main thread recieved signal: %d\n", isignal);
        }
    }
