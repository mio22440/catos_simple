...
#define ADDR_OF_VAR(x) (&(x))
FATFS fs_var[_VOLUMES];
FIL file_var;
FIL ftemp_var;
u8  fatbuf_var[512];
...
u8 exfuns_init(void)
{
	...
	{
		fs[i]=(FATFS*)ADDR_OF_VAR(fs_var[i]);	//为磁盘i工作区申请内存	
		if(!fs[i])break;
	}
	file=(FIL*)ADDR_OF_VAR(file_var);		//为file申请内存
	ftemp=(FIL*)ADDR_OF_VAR(ftemp_var);		//为ftemp申请内存
	fatbuf=(u8*)fatbuf_var;				//为fatbuf申请内存
